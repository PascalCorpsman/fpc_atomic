...
# Extract app name from existing fly.toml or fly.toml.example
...
if [[ -f "${FLYIO_DIR}/fly.toml" ]]; then
  ...
  if [[ -n "${EXTRACTED_APP}" && "${EXTRACTED_APP}" != "your-app-name-here" ]]; then
    APP_NAME="${EXTRACTED_APP}"
    echo "✓ Using app name from fly.toml: ${APP_NAME}"
  fi
elif [[ -f "${FLYIO_DIR}/fly.toml.example" ]]; then
  # Try to extract app name from fly.toml.example
  EXTRACTED_APP=$(grep -E '^app\s*=' "${FLYIO_DIR}/fly.toml.example" | head -1 | sed -E 's/^app\s*=\s*["'\'']?([^"'\'']+)["'\'']?.*/\1/' | tr -d ' ')
  if [[ -n "${EXTRACTED_APP}" && "${EXTRACTED_APP}" != "your-app-name-here" ]]; then
    APP_NAME="${EXTRACTED_APP}"
    echo "✓ Using app name from fly.toml.example: ${APP_NAME}"
  fi
fi

if [[ "${APP_NAME}" == "your-app-name-here" ]]; then
  echo "⚠️  Warning: Using placeholder app name 'your-app-name-here'"
  echo "   Please create fly.toml with your app name, or edit fly.toml.example"
  echo ""
fi
